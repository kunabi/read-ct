...
(def (parse-row fields row txn)
  ...
  (let* ((h (make-hash-table-eq)); 'size: 14))
    ...
    (hash-put! h 'aws-region aws-region)
    (hash-put! h 'event-id event-id)
    (hash-put! h 'event-name event-name)
    (hash-put! h 'event-source event-source)
    (hash-put! h 'event-time event-time)
    (hash-put! h 'event-type event-type)
    (hash-put! h 'recipient-account-id recipient-account-id)
    (hash-put! h 'request-id request-id)
    (hash-put! h 'request-parameters request-parameters)
    (hash-put! h 'response-elements response-elements)
    (hash-put! h 'source-ip-address source-ip-address)
    (hash-put! h 'user-agent user-agent)
    (hash-put! h 'user-identity user-identity)
